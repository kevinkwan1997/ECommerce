...
const testProductsByCategory: Item[] = [
...
describe('FakeStoreApiService', () => {
  ...
  beforeEach(() => {
    ...
    TestBed.configureTestingModule({
      ...
      providers: [FakeStoreApiService, { provide: LogService, useValue: spy}]
    });
  ...
  describe('Testing HTTP', () => {
    it('should get full list of products', waitForAsync (() => {
      service.getAllProducts().subscribe(products => {
        expect(service.getAllProducts).toHaveBeenCalled();
        expect(products).toEqual(testData)
      })

      let req = httpTestingController.expectOne(`${baseUrl}/products`)
      expect(req.request.method).toEqual('GET')
      httpTestingController.verify();
    }))

    it('should get individual product', waitForAsync(() => {
      service.getProduct(1).subscribe(product => {
        expect(service.getProduct).toHaveBeenCalled();
        expect(product).toEqual(testProduct)
      })
      let req = httpTestingController.expectOne(`${baseUrl}/products/1`)
      expect(req.request.method).toEqual('GET')
      httpTestingController.verify();
    }))

    it('should get cagetories', waitForAsync(() => {
      service.getCategories().subscribe(categories => {
        expect(service.getCategories()).toHaveBeenCalled();
        expect(categories).toEqual(testCategories)
      })
      let req = httpTestingController.expectOne(`${baseUrl}/products/categories`)
      expect(req.request.method).toEqual('GET')
      httpTestingController.verify();
    }))

    it('should get products by category', waitForAsync(() => {
      service.getProductsByCategory('jewelery').subscribe(products => {
        expect(service.getProductsByCategory('jewelery')).toHaveBeenCalled();
        expect(products).toEqual(testProductsByCategory)
      })
      let req = httpTestingController.expectOne(`${baseUrl}/products/category/jewelery`)
      expect(req.request.method).toEqual('GET')
      httpTestingController.verify();
    }))
  })
